package io.github.christophknabe.multex.core;  //MultexExceptionTest.java
...
public class MultexExceptionTest extends MultexAssert {
	...
	public static final String EXAMPLE_MESSAGE_TEXT_PATTERN = "Filename length {0} exceeds allowed {1}";
...
static class MyExc extends Exc {
	...
	@TextPattern(EXAMPLE_MESSAGE_TEXT_PATTERN)
	static class AnnotatedEmptyExc extends Exc {}

	@TextPattern(EXAMPLE_MESSAGE_TEXT_PATTERN)
	static class AnnotatedEmptyFailure extends Failure {}

	@Test
	public void annotationOnly_provides_defaultMessageTextPattern(){
		//GIVEN
		final MultexException[] annotatedExceptions = {new AnnotatedEmptyExc(), new AnnotatedEmptyFailure()};
		for (final MultexException annotatedException: annotatedExceptions) {
			//WHEN
			final String result = annotatedException.getDefaultMessageTextPattern();
			//THEN
			assertEquals(EXAMPLE_MESSAGE_TEXT_PATTERN, result);
		}
	}

	@TextPattern(EXAMPLE_MESSAGE_TEXT_PATTERN)
	static class AnnotatedAndInstanceTextExc extends Exc {
		AnnotatedAndInstanceTextExc(final String defaultMessageTextPattern){
			super(defaultMessageTextPattern);
		}
	}

	@TextPattern(EXAMPLE_MESSAGE_TEXT_PATTERN)
	static class AnnotatedAndInstanceTextFailure extends Failure {
		AnnotatedAndInstanceTextFailure(final String defaultMessageTextPattern){
			super(defaultMessageTextPattern);
		}
	}

	@Test
	public void instance_supersedes_annotation_textPattern(){
		//GIVEN
		final String instanceTextPattern = "This is the instance textPattern";
		final MultexException[] annotatedExceptions = {
				new AnnotatedAndInstanceTextExc(instanceTextPattern),
				new AnnotatedAndInstanceTextFailure(instanceTextPattern)
		};
		for (final MultexException annotatedException: annotatedExceptions) {
			//WHEN
			final String result = annotatedException.getDefaultMessageTextPattern();
			//THEN
			assertEquals(instanceTextPattern, result);
		}
	}
